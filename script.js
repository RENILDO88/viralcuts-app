document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA DO FAQ (Funciona em qualquer página que tiver o FAQ) ---
    ...
    if (editorContainer) {
        ...
        // Simular Adicionar Texto
        if (addTextBtn) {
            addTextBtn.addEventListener('click', () => {
                const videoWrapper = document.getElementById('video-player-wrapper');
                if (videoWrapper.querySelector('.draggable-text')) return; // Evita adicionar múltiplos textos
                const sampleText = document.createElement('div');
                sampleText.textContent = 'Edite este texto!';
                sampleText.className = 'draggable-text';
                sampleText.contentEditable = true;
                videoWrapper.appendChild(sampleText);
            });
        }
